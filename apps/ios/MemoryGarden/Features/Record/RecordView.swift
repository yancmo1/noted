...
struct RecordView: View {
    ...
    @State private var title = "Untitled Recording"
    @State private var mode = "private_thought"
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    }.buttonStyle(.plain).disabled(!isRecording && needsConsent && !consentAcknowledged).accessibilityLabel(isRecording ? "Stop recording" : "Start recording")
                    if isRecording { HStack(spacing: 12) { Button { model.audioRecorder.markMoment().map { moments.append($0) } } label: { Label("Mark Moment", systemImage: "bookmark.fill") }.buttonStyle(.borderedProminent); Button { model.audioRecorder.state == .recording ? model.audioRecorder.pause() : model.audioRecorder.resume() } label: { Label(model.audioRecorder.state == .recording ? "Pause" : "Resume", systemImage: model.audioRecorder.state == .recording ? "pause.fill" : "play.fill") }.buttonStyle(.bordered) } }
    ...
    private func toggleRecording() {
        ...
        else { model.errorMessage = nil; Task { do { try await model.audioRecorder.start() } catch { model.errorMessage = error.localizedDescription } } }
    }
}
