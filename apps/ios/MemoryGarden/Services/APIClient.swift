...
final class APIClient {
    ...
    func login(password: String) async throws {
        let _: LoginResponse = try await send(path: "/api/auth/login", method: "POST", body: try JSONEncoder().encode(["password": password]))
    }

    func logout() async throws { let _: LoginResponse = try await send(path: "/api/auth/logout", method: "POST") }
    ...
    func downloadAudio(sourceId: String) async throws -> URL {
        let request = try makeRequest(path: "/files/\(sourceId)")
        do {
            let (temporary, response) = try await session.download(for: request)
            guard let http = response as? HTTPURLResponse else { throw APIError.invalidResponse }
            guard (200..<300).contains(http.statusCode) else { throw http.statusCode == 401 ? APIError.unauthorized : APIError.server(http.statusCode, "Audio download failed") }
            return temporary
        } catch let error as APIError { throw error } catch { throw APIError.offline(error) }
    }

    func uploadRecording(_ recording: LocalRecording) async throws -> UploadResponse {
