...
- **Xcode Cloud/App Store Connect setup:** commit/push the repository changes, create or select the `Noted by Shepswork.` app record for `com.shepswork.noted`, register the iPhone/Watch/Share Extension bundle IDs and application group, choose the latest stable non-beta Xcode runner, and start an Archive workflow using the shared `Noted` scheme. These account-side steps cannot be encoded in the repository.
