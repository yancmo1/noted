...
- **Xcode Cloud/TestFlight export:** the `Noted by Shepswork` app record, workflow, stable `Latest Release` runner, shared `Noted` archive scheme, and iPhone/Share identifiers are in place; the missing Watch App ID is now registered. The next gate is a successful managed-signing export and TestFlight processing, followed by publishing the local repository changes to the connected `dev` branch.
- **Cloud account session:** the previous export also reported that the Xcode Cloud session could not authenticate with App Store Connect. Build 4 was started after the Watch identifier was registered; if it repeats the authentication failure, the remaining recovery step is in App Store Connect/Xcode Cloud account access rather than the app source.
